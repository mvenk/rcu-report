...
\begin{figure}[tph]
\begin{center}
\caption{The example illustrates how two concurrent updaters can corrupt the sate of the freelist. The initial state of the freelist is as shown.The first parameter is the route, the second parameter asks the function to use the route supplied even if a route previously existed for that IP Address. The third parameter \emph{old\_route} retrieves a preexisting route for that IP if it is not set to NULL. The fourth parameter is the error handler.}
\includegraphics[scale=0.4]{../images/diagrams/freelistrace1}
\begin{tabular}{|p{2.2in}|p{2.2in}|}
\hline
\textbf{Thread 1} & \textbf{Thread 2}\\
\hline
Thread 1 calls \emph{add\_route(X,1,NULL,NULL), X.addr =A, Y.mask = M}. 
&
Thread 2 calls \emph{add\_route(Y,1,NULL,NULL) Y.addr = A, Y.mask =M}.\\
\hline
\begin{verbatim}{fontsize=\small}
 \verb$int found = (_vfree < 0 ? _v.size() : _vfree), last_key;
\end{verbatim}
 Obtains a value $found = 3$
&
 $int found = (\_vfree < 0 ? \_v.size() : \_vfree), last\_key;$
 Obtains a value $found = 3$\\
\hline
\begin{tabular}{p{1.8in}}
Executes:\\
...\
verb$_v[found].extra = -1;$\\
...\\
The state of the freelist is shown in the figure. \\\\
